...
export default function App() {
    ...
    return (
        <div
            className="flex flex-col h-screen bg-white text-gray-900 font-sans selection:bg-gray-200 selection:text-black"
            ...
            {/* Sidebar */}
            <div
                className={`fixed top-0 left-0 h-full w-72 bg-white shadow-2xl z-50 transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col`}>
                    ...
                    {conversations.map(conv => (
                        ...
                        >
                            ...
                            <button
                                ...
                                title="Delete chat"
                            >
                                <Trash2 size={16} className={conv.id === currentId ? '' : 'text-gray-500'}
                                        style={conv.id === currentId ? {color: themeColor} : {}}/>
                            </button>
                        </div>
                    ))}
                </div>
            ...
            {/* Top App Bar */}
            <header
                className="flex items-center justify-between px-2 py-2 bg-white text-gray-900 shrink-0 z-10 border-b border-gray-100">
                ...
                <div className="flex items-center gap-1 relative">
                    ...
                    {isColorPickerOpen && (
                        <div
                            className="absolute top-full right-0 mt-2 w-64 bg-white rounded-2xl shadow-xl border border-gray-100 p-4 z-50">
                ...
                <div className="max-w-3xl mx-auto flex flex-col gap-6">
                    {messages.map((msg) => (
                        ...
                        >
                            ...
                            {msg.role === 'agent' && (
                                <div
                                    className="shrink-0 w-8 h-8 rounded-full flex items-center justify-center mt-1 shadow-sm text-white"
                                    style={{backgroundColor: themeColor}}>
                                    <Sparkles size={16}/>
                                </div>
                            )}
                            ...
                            <div className={`px-5 py-3.5 text-[15px] leading-relaxed shadow-sm ${
                                msg.role === 'user'
                                    ? 'text-white rounded-[24px] rounded-tr-[4px]'
                                    : 'bg-gray-50 border border-gray-100 text-gray-900 rounded-[24px] rounded-tl-[4px]'
                            }`}
                    ...
                    {isLoading && (
                        <motion.div initial={{opacity: 0}} animate={{opacity: 1}}
                            ...
                            <div
                                className="bg-gray-50 border border-gray-100 text-gray-900 rounded-[24px] rounded-tl-[4px] px-5 py-4 flex items-center gap-3 shadow-sm">
            ...
            <footer className="bg-white p-4 shrink-0 pb-6 border-t border-gray-100"
                    onClick={() => setIsColorPickerOpen(false)}>
                <div className="max-w-3xl mx-auto relative flex flex-col gap-2">
                    ...
                    {isInputExpanded && (
                        ...
                        >
                            ...
                            >
                                <div
                                    className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform ${isStreaming ? 'left-7' : 'left-1'}`}/>
                            </button>
                        </motion.div>
                    )}

                    <div
                        className="flex items-end gap-2 bg-gray-50 border border-gray-200 rounded-[28px] p-2 focus-within:bg-white focus-within:border-gray-300 focus-within:shadow-sm transition-all duration-300">
                        <button
                        ...
                        <textarea
                            ...
                            className="flex-1 max-h-32 min-h-[48px] bg-transparent border-none focus:ring-0 resize-none py-3 text-gray-900 placeholder:text-gray-400 text-[15px] leading-relaxed outline-none"
                            rows={1}
                        />
                        <button
                            onClick={handleSend}
